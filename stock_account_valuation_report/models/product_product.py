...
class ProductProduct(models.Model):
    ...
    def _get_prsit_all_base_domain(self, company_id=False):
        domain = [
            ('state', '=', 'done'),
            '|',
                '&',
    ...
    def _sum_remaining_values_prsit(self, product):
        StockMove = self.env['stock.move']
        domain = [('product_id', '=', product.id)] + self._get_prsit_all_base_domain()
        moves = StockMove.search(domain)
        return sum(moves.stock_valuation_layer_ids.mapped('remaining_value')), moves
    ...
    def _compute_inventory_value(self):
        ...
        stock_move_domain = [
            ...
            ('date', '<=', to_date)] + self._get_prsit_all_base_domain()
            ...
            if product.cost_method in ['standard', 'average']:
                ...
                if to_date:
                    ...
                    product.stock_value, moves = \
                        self._sum_remaining_values_prsit(product)
#                        product._sum_remaining_values()
    ...
    def action_view_stock_moves(self):
        self.ensure_one()
        to_date = self.env.context.get('to_date')
        tree_view_ref = self.env.ref('stock.view_move_tree')#V14
